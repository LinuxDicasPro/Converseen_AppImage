...
APP=converseen
BIN="$APP"
DEPENDENCES="bzip2 cairo fftw fontconfig freetype2 gcc-libs glib2 glibc lcms2 liblqr libtool libpng libraqm libxext libxml2 xz zlib djvulibre ghostscript jbigkit libheif libjpeg-turbo libjxl libraw librsvg libtiff libultrahdr libwebp libwmf libzip ocl-icd openexr openjpeg2 pango hicolor-icon-theme imagemagick qt6-base libxcb libxkbcommon libxkbcommon-x11"
...
cd $APPDIR
desktopfile=~/.local/share/applications/net.fasterland.converseen.desktop
iconfile=~/.local/share/pixmaps/$prg.png

test ! -e "$desktopfile" && cp net.fasterland.converseen.desktop ~/.local/share/applications/
...
LIBSAVED="SAVELIBSPLEASE $lib_browser_launcher qt6 ImageMagick-7.1.1 libdjvulibre.so" # Enter here keywords or file/directory names to save in /usr/lib
...
mv ./Converseen-x86_64.AppImage ./Converseen-"$VERSION"-x86_64.AppImage
